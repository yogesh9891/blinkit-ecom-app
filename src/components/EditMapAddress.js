...
export default function EditMapAddress(props) {
  ...
  useEffect(() => {
    if (props.route.params.address) {
      if(props.route.params.address?.location){
        let  locationOBJ = {};

        console.log(props.route.params.address,"props.route.params.addressprops.route.params.addressprops.route.params.addressprops.route.params.address")
        setAddressName(props.route.params.address.addressName)
        setAddress(props.route.params.address.addressName);
        setAddresId(props.route.params.address._id)
        locationOBJ.latitude = parseFloat(props.route.params.address?.location?.latitude)
        locationOBJ.longitude = parseFloat(props.route.params.address?.location?.longitude)
        locationOBJ.latitudeDelta= 0.00922 * 1.5; locationOBJ.longitudeDelta= 0.00421 * 1.5;
        console.log(locationOBJ,"locationOBJlocationOBJlocationOBJlocationOBJlocationOBJlocationOBJlocationOBJ")
        setRegionObj(locationOBJ);
        Geolocation.getCurrentPosition(position => setNewRegionObj({ latitude: position.coords.latitude, longitude: position.coords.longitude, latitudeDelta: 0.00922 * 1.5, longitudeDelta: 0.00421 * 1.5 }));
        
      } else {
        // Geolocation.getCurrentPosition(position => setRegionObj({ latitude: position.coords.latitude, longitude: position.coords.longitude, latitudeDelta: 0.00922 * 1.5, longitudeDelta: 0.00421 * 1.5 }));
      }
    }
...
useEffect(() => {
  if(regionObj?.longitude && regionObj?.latitude){
    console.log(regionObj,"regionObjregionObjregionObjregionObjregionObjregionObj")
  ...
  const handleSubmit = async () => {
    try {
      console.log('HANDLING SUBMIT', props.route.params.address);
      let regionParams = regionObj;
        regionParams.addressName = address;
    navigation.navigate("SaveLocation",{addressResponse:{name:address,obj:regionParams,postal_code:postalCode,location:location,addressId:props.route.params.address._id}})
  ...
  return (
    <SafeAreaView style={{ backgroundColor: '#fff', flex: 1 }}>
      <Header stackHeader={true} screenName={"Edit Address"} rootProps={props} />
      {/* <Image source={require('../Assets/image/m.png')} style={{ width: '100%', height: hp(65) }} /> */}
      {regionObj && regionObj?.latitude && (
        <MapView style={{ width: wp(100), height: hp(70) }} region={newregionObj}>
          <Marker
            key={"12"}
            draggable
