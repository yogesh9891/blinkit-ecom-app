...
import { addVacationApi } from '../services/Vacation.service';
import { toastError, toastSuccess } from '../utils/toastError';
import { getAuth } from '../utils/auth';
export default function Addvacation(props) {
    ...
    const addVacation = async () => {
        try {
            ...
            // }
            let obj = {
                title: 'new', startDate, endDate, userId: user?._id
            }
            console.log(obj, "objobjobjobjobjobjobjobj")
            let { data: res } = await addVacationApi(obj);
            if (res.success) {
        ...
        <>
            ...
            <View style={styles.vacatftrfixed}>
                ...
                <Pressable onPress={() => addVacation()}>
                    <Text style={[styles.btncontun, styles.textcenter, styles.txtconti, styles.fontw6]}>Submit</Text>
                </Pressable>
            </View>
            ...
            <Modal
                ...
                <View style={styles1.centeredView}>
                    <View style={styles1.modalView}>
                        ...
                        {
                            date == 'start' ? (<DatePicker mode="date" date={startDate} minimumDate={new Date()} open={true} onDateChange={(date) => {
                                setStartDate(date)
                            }}
